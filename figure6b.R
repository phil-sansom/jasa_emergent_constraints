...
round(quantile((samples[,"beta"] - 1)*(samples[,"yh"] - samples[,"muh"]),
               probs = c(0.05,0.95)), 1)
round(sd(samples[,"yf"] - samples[,"yh"]), 1)
round(sd(samples0[,"yf"] - samples0[,"yh"]), 1)
round(sd(samples[,"yf"] - samples[,"yh"])/sd(samples0[,"yf"] - samples0[,"yh"]), 2)
...
#fit = apply(yr, 2, mean)
fit = mean(samples[,"muf"]) - xx + mean(samples[,"beta"])*(xx - mean(samples[,"muh"]))
...
## Open PDF device
pdf(file = "figure6b.pdf", width = 6.0, height = 3.7)
...
muz = mean(z)
beta = samples[,"muf"] - muz + samples[,"beta"]*(muz - samples[,"muh"]) +
  rnorm(1e3, 0, sqrt(1/samples[,"tauf"] + 1/(samples[,"phia"]*samples[,"taua"])))
beta = mean(beta)
segments(x0 = muz, y0 = ylim[1], y1 = beta, col = "blue", lty = "dashed", lwd = 2)
segments(x0 = xlim[1], x1 = muz, y0 = beta, col = "blue", lty = "dashed", lwd = 2)
...
segments(x0 = mean(yh), y0 = ylim[1], y1 = mean(yr), col = "red", lty = "dashed", lwd = 2)
segments(x0 = xlim[1], x1 = mean(yh), y0 = mean(yr), col = "red", lty = "dashed", lwd = 2)
...
murd = density(mur)
# lines(xlim[1] + rho*yrd$y, yrd$x, col = "black")
segments(x0 = mean(muh), y0 = ylim[1], y1 = mean(mur), col = "black", lty = "dashed", lwd = 2)
segments(x0 = xlim[1], x1 = mean(muh), y0 = mean(mur), col = "black", lty = "dashed", lwd = 2)

# abline(v = xlim[1],col=2)
# abline(v = xlim[2],col=2)

mtext("(b)", side = 3, line = 0, adj = -0.15)
