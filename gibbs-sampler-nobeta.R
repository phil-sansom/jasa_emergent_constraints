...
gibbs.sampler = function(n, summary.statistics, kappa, initial.values,
    ...
    xfm  = rnorm(nxhm, muf + xhm - muh, sqrt(1/tauf))
    ...
    samples = matrix(data = NA, nrow = n, ncol = 13 + 4*m,
                     dimnames = list(NULL, c("muh","muf","tauh","tauf",
    ...
    for (i in 1:n) {
        ...
        mean    = tauh*muh + tauf*(xfm - muf + muh) + taum*nxhmr*mxhmr
        var     = 1/(tauh + tauf + taum*nxhmr)
        xhm     = rnorm(m, mean*var, sqrt(var))

        ## Update future model climates
        mean    = tauf*(muf + xhm - muh) + phim*taum*nxfmr*mxfmr
        ...
        for (j in 1:m)
            rate[j] = rate[j] + sum((xhmr[[j]] - xhm[j])^2) +
                phim[j]*sum((xfmr[[j]] - xfm[j])^2)
